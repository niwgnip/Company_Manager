...
class Company:
	...
	def hire_new(self):
		...
		if new_pos == 1:
			...
			new_working_time = int(input ("How many hours will he work per week?"))
			...
			new_salary = input("What will be his/her salary?")
			new_emp = employee.SalariedEmployee(new_name, new_salary)
			print(f"You hired new employee - {new_name} on a position -> {new_emp.position}")
		elif new_pos == 3:
			new_salary = input("What will be his/her salary?")
			new_commission = input("What will be his/her commission?")
	...
	def rise_employee(self, emp_list,emp_dict):
		...
		for i in range(len(emp_list)):
			if emp_list[i].employee_name == name and emp_list[i].position == 'Hourly Employee':
				new_salary = input("Provide new salary after promotion to Salaried Employee (per week)")
				new_name = name
				...
				new_salary = input("Provide new salary after promotion to Manager (per week)")
				new_commission = input ("Provide commission for Magager: ")
				new_name = name
				new_emp = employee.Manager(new_name,new_salary,new_commission)
				emp_list.insert(i+1,new_emp)
				emp_dict.update({new_emp.employee_name:new_emp.position})
				...
				new_salary = input("Provide new salary after promotion to Executive (per week)")
				new_commission = input ("Provide commission for Executive: ")
				new_name = name
				new_emp = employee.Executive(new_name,new_salary,new_commission)
				emp_list.insert(i+1,new_emp)
				emp_dict.update({new_emp.employee_name:new_emp.position})
